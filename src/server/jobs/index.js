...
export const getJobsByEvent = async (req) => { 
    return await handleSQLRequest(`SELECT * FROM  brothers, events, jobs WHERE ( jobs.event_id = events.id AND brothers.id = jobs.brother_id AND events.id = '${req.query.id}') ; `)
}
